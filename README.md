...
- 开机自启设置；
- 开机自启后支持隐藏启动；
- 系统托盘（显示/隐藏全部便签、新建便签、设置、退出）；
- 关闭窗口默认隐藏到托盘；
- 全局快捷键（Windows 默认 Alt+N 显示/隐藏全部便签、Alt+Shift+N 新建便签；macOS 默认 CmdOrCtrl+Option+N 显示/隐藏全部便签、CmdOrCtrl+Option+Shift+N 新建便签）；
- 应用内快捷键（Ctrl+S/T/,// 等）；
...
- 工具栏便签列表：可查看全部便签，并单独显示、隐藏或删除；
- 当前便签可确认后删除，删除后会从 `notes.json` 移除；
- `Ctrl+S` 只导出当前聚焦的便签；
- 当前编辑模式内支持撤销/重做；
...
关闭某张便签时，它会隐藏并保存自己的状态，不会删除内容。工具栏垃圾桶按钮才是真正删除：确认后会从 `notes.json` 移除当前便签，并销毁对应的独立窗口。

托盘里的“显示/隐藏全部便签”和全局快捷键使用同一套逻辑：如果当前有任意便签可见，则隐藏所有可见便签；如果当前没有便签可见，则显示 `notes.json` 中所有未删除的便签。这样手动关闭/隐藏过的便签也能通过快捷键或托盘恢复。`Ctrl+S` 只作用于当前聚焦的便签，不会导出其他窗口的内容。

工具栏里的便签列表会显示当前保存的全部便签。每张便签旁边有显示/隐藏按钮和删除按钮，适合在多便签变多时快速找回、收起或清理某一张便签。
...
- 将托盘和全局快捷键统一为“显示/隐藏全部便签”
- 支持删除当前便签，删除后从 `notes.json` 移除并销毁对应窗口
- 新增工具栏便签列表，支持单张便签显示、隐藏和删除
...
- 全局快捷键：Windows 默认 `Alt+N` 显示便签、`Alt+Shift+N` 新建便签；macOS 默认 `CmdOrCtrl+Option+N` 显示便签、`CmdOrCtrl+Option+Shift+N` 新建便签
